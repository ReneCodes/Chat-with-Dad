...
$(document).ready(function () {
  ...
  */
	...
	function introText() {
		...
		getRandomDadResponse('But I will respond to every text. Promise!\nGreetings, Dad');
	}
    ...
    - Hallo wie geht es dir. Hast du heute schon gegessen?
  */
  ...
  */
	$('#input').on('input', function (event) {
		...
		let maxLetterPerLine = Math.ceil(0.092 * (event.target.offsetWidth - event.target.offsetLeft * 2));
  ...
  */
	$('.sent-text-btn').on('click', function () {
		if ($('#input')[0].value.length >= 1) {
			buildMessageBlock(getUserMessage() || 'Default Message \n Something went wrong', 'right', true);
		}
		focusOnInput();
	});
  ...
  */
	function buildMessageBlock(message, position = 'left', human = false) {
		...
		$timestamp.addClass('timestamp').html(`<p>${getTimestamp()}</p>`).appendTo($block);
  ...
  */
	function getTimestamp() {
		...
		const timeStamp = `${date[2]}.${date[1]}.${date[3].slice(2, 4)}&nbsp&nbsp${time.slice(0, 5)}`;
  ...
  */
	function getRandomDadResponse(text = '') {
		...
		if (text) {
			...
			randomText = randomDadTexts[Math.floor(Math.random() * randomDadTexts.length)];
